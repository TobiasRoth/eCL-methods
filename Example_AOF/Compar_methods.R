...
dat$NKat <- "5-10"
dat$NKat[dat$Ndep >= 10] <- "10-15"
dat$NKat[dat$Ndep >= 15] <- "15-20"
dat$NKat[dat$Ndep >= 20] <- "20-25"
dat$NKat[dat$Ndep >= 25] <- "25-30"
dat$NKat[dat$Ndep >= 30] <- "30-33"
dat$NKat <- factor(dat$NKat, levels = c("5-10", "10-15", "15-20", "20-25", "25-30", "30-33"))
table(dat$NKat)
...
# Make plot with indicator species
pd <- tibble(
  x = c(1, 2, 2, 1),
  y = c(0, 0, 30, 30)
)
plote <- dat %>% 
  ...
  dplyr::summarise(
    N = n(),
    meanSR = mean(SR_indi),
    lo = t.test(SR_indi)$conf.int[1],
    up = t.test(SR_indi)$conf.int[2]
  ) %>% 
  ggplot(aes(x = NKat, y = meanSR)) +
  geom_point() +
  geom_errorbar(aes(ymin = lo, ymax = up), width = 0.2) +
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# (b) TITAN approach: all species  ----
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

# Prepare site x species matrix
speciesmat <- dat[, 8:ncol(dat)]
...
#     ncpus = 4)
# save(titan_all, file = "Example_AOF/titan_all.RData")
load("Example_AOF/titan_all.RData")
titan_all$sumz.cp["sumz-", c("cp", "0.05", "0.95")] %>% round(1)

# Make plot
d <- titan_all[["sppmax"]] %>% 
  as_tibble() %>% 
  mutate(aID_SP = row.names(titan_all[["sppmax"]])) %>% 
  filter(filter == 1) %>% 
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# (f) TITAN approach: indicator species  ----
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

# Load and prepare site x species matrix
# Prepare site x species matrix
speciesmat <- dat[, 
                  c("QUERPETR", "BETUPUBE", "CORYAVEL", "ILEXAQUI", "LONIPERI", "SORBAUCU",
                    "VACCMYRT", "BLECSPIC", "LUZUSYLV", "OXALACET", "HYACNONS", "POLYVULG",
                    "DICRSCOP", "DIPLALBI", "EURHSTRI", "HYLOBREV", "HYPNCUPR", "HYPNJUTL",
                    "ISOTMYOS", "MNIUHORN", "POLYFORM", "PSEUELEG", "RHYTLORE", "SACCVITI",
                    "SCAPGRAC")
]
speciesmat <- speciesmat[, apply(speciesmat, 2, function(x) sum(x > 0)) > 5] 
...
#     nBoot = 500, # 500 oder 1000 für die echte Analyse
#     ncpus = 4)
# save(titan_oligo, file = "Example_AOF/titan_indi.RData")
load("Example_AOF/titan_indi.RData")
titan_oligo$sumz.cp["sumz-", c("cp", "0.05", "0.95")] %>% round(1)

# Make plot
d <- titan_oligo[["sppmax"]] %>% 
  as_tibble() %>% 
  mutate(aID_SP = row.names(titan_oligo[["sppmax"]])) %>% 
  filter(filter == 1) %>% 
  mutate(aID_SP = fct_reorder(aID_SP, `50%`))
pd <- tibble(
...
mod <- glm(SR ~ Ndep, data = dat, family = poisson) 
summary(mod)
nsim <- 1000
...
# Indicator species
mod <- glm(SR_indi ~ Ndep, data = dat, family = poisson) 
summary(mod)
nsim <- 1000
post <- sim(mod, nsim)@coef
...
datjags <- list(SR = SR, nsite = nsite, N = N)
jagres <- jags.model('Example_AOF/Change-point_model.R',data = datjags, n.chains = t.n.chains, inits = inits, n.adapt = t.n.burnin)
params <- c("CL", "SRpre")
...
# Make plot for indicator species
SR <- as.integer(dat$SR_indi)
datjags <- list(SR = SR, nsite = nsite, N = N)
jagres <- jags.model('Example_AOF/Change-point_model.R',data = datjags, n.chains = t.n.chains, inits = inits, n.adapt = t.n.burnin)
params <- c("CL", "SRpre")
mod <- coda.samples(jagres, params, n.iter=t.n.iter, thin=t.n.thin)
...
pdf("Example_AOF/Fig_AOF_method_comparison.pdf", width = 14, height = 8)
